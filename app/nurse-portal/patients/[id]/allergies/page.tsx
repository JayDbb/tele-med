...
export default function PatientAllergiesPage() {
  ...
  const handleAddAllergy = () => {
    ...
    const entry = {
      id: `${Date.now()}-${newAllergy.name}`,
      ...newAllergy,
      recordedAt: new Date().toISOString()
    }
    const nextAllergies = [entry, ...allergies]
    setAllergies(nextAllergies)
    PatientDataManager.savePatientSectionList(patientId, 'allergies', nextAllergies)
    PatientDataManager.logActionAuto(patientId, 'update', 'allergies', {
      notes: `Added allergy: ${newAllergy.name}`
    })
    setNewAllergy({
      name: '',
      severity: 'Moderate',
            ...
            <div className="px-5 py-4 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-white focus:ring-primary focus:border-primary"
