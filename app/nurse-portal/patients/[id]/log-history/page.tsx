...
export default function PatientLogHistoryPage() {
  ...
  const renderDetails = (log: any) => {
    if (!log) return 'No details recorded'
    const notes = `${log.notes || ''}`.trim()
    if (notes) return notes
    if (!log.changes) return 'No details recorded'
    const changes = log.changes
    if (typeof changes === 'string') return changes
    const before = changes.before
    const after = changes.after
    if (after?.items && Array.isArray(after.items)) {
      return `${after.items.length} item${after.items.length === 1 ? '' : 's'} saved`
    }
    if (after?.status && before?.status && before.status !== after.status) {
      return `Status: ${before.status} → ${after.status}`
    }
    if (after?.status && !before?.status) {
      return `Status set to ${after.status}`
    }
    if (after?.name || after?.dob || after?.mrn) {
      const parts = []
      if (after.name && after.name !== before?.name) parts.push(`Name: ${after.name}`)
      if (after.dob && after.dob !== before?.dob) parts.push(`DOB: ${after.dob}`)
      if (after.mrn && after.mrn !== before?.mrn) parts.push(`MRN: ${after.mrn}`)
                  ...
                  <tbody className="divide-y divide-slate-200 dark:divide-gray-700">
                    {filteredLogs.length === 0 ? (
                      ...
                      filteredLogs.map((log, index) => (
                        <tr key={`${log.id || 'log'}-${index}`} className="hover:bg-slate-50 dark:hover:bg-gray-700/50">
