...
export default function PatientVitalsPage() {
  ...
  const [exportEmail, setExportEmail] = useState(patient?.email || '')
  const [exportPhone, setExportPhone] = useState(patient?.phone || '')
  ...
  const downloadVitalsReport = () => {
    const rows = [
      ['Recorded At', 'Blood Pressure', 'Heart Rate', 'SpO2', 'Notes'],
    ...
    link.href = url
    link.download = `${patient?.name || 'patient'}-vitals-report.csv`
  ...
  const handleSendExport = async () => {
    ...
    const exports = PatientDataManager.getPatientSectionList(patientId, 'vitals-export')
    PatientDataManager.savePatientSectionList(patientId, 'vitals-export', [payload, ...exports])
    PatientDataManager.logActionAuto(patientId, 'export', 'vitals', {
      notes: `Exported vitals via ${exportMethod}.`
    })

    if (process.env.NEXT_PUBLIC_API_ENABLED === 'true') {
      try {
        const response = await fetch('/api/vitals/export', {
          method: 'POST',
    ...
    setExportStatus(`Sent via ${exportMethod === 'email' ? 'email' : 'text'}.`)
