...
const NewVisitForm = ({ patientId }: NewVisitFormProps) => {
  ...
  useEffect(() => {
    if (!isNewPatient) {
      setExistingPatient(PatientDataManager.getPatient(patientId))
    }
  ...
  const patient = isNewPatient ? {
    ...
    name: existingPatient?.name || 'Not provided',
    dob: existingPatient?.dob || 'Not provided',
    mrn: existingPatient?.mrn || 'Not provided',
    allergies: existingPatient?.allergies || 'None'
  }
