...
import { createVisit, getPatient, transcribeAudio, updateVisit, upsertVisitNote } from "../../../../lib/api";
...
export default function PatientVisitPage() {
  ...
  const processAudioFile = async (mp3File: File) => {
    ...
    try {
      const transcriptionResult = await transcribeAudio(upload.path, newVisit.id);
      ...
      if (transcriptionResult.structured) {
        ...
        if (structured.current_symptoms && Array.isArray(structured.current_symptoms)) {
          const symptomsText = structured.current_symptoms
            .map((s: { symptom: string; characteristics?: string }) => s.symptom)
                ...
                <div className="border-b border-[#e7edf3] px-4 bg-[#f6f7f8]/50">
                  <div className="flex gap-6">
                    <button 
                      ...
                      className={`flex flex-col items-center justify-center border-b-[3px] gap-1 pb-3 pt-4 px-2 transition-colors ${
                        activeTab === "record" 
                        ...
                        <div className="space-y-4">
                          {transcription.structured.current_symptoms && Array.isArray(transcription.structured.current_symptoms) && (
                            <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                              ...
                              <div className="space-y-2">
                                {transcription.structured.current_symptoms.map((symptom: { symptom: string; characteristics?: string }, idx: number) => (
